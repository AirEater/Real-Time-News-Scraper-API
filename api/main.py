...
# Import the core search logic and utilities
from core.search.search_logic import search_on_demand, search_standard
...
async def startup_event():
    ...
    try:
        ...
        if not utility.has_collection(milvus_config.collection_name):
            raise RuntimeError(f"Milvus collection '{milvus_config.collection_name}' not found.")
        
        milvus_collection = Collection(milvus_config.collection_name)
        ...
        app.state.mongo_collection = db[db_config.mongo_collection]
...
async def search_standard_endpoint(request: SearchRequest):
    """
    Perform a standard semantic search for news articles.
    This endpoint uses pre-processed data and does not perform on-demand classification.
    """
    try:
        results = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: search_standard(
        ...
        if results and 'results' in results:
            return {"status": "success", "search_metadata": results.get('search_metadata', {}), "count": len(results['results']), "data": results['results']}
        else:
            return {"status": "no_results", "search_metadata": results.get('search_metadata', {}), "count": 0, "data": []}
    except Exception as e:
        logger.error(f"An error occurred during standard search: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="An internal error occurred during the search process.")

@app.post("/search/on-demand")
async def search_on_demand_endpoint(request: SearchRequest):
    """
    Perform an advanced semantic search with on-demand classification and summarization for any articles that have not been processed.
    """
    try:
        results = await asyncio.get_event_loop().run_in_executor(
            None,
        ...
        logger.error(f"An error occurred during on-demand search: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="An internal error occurred during the search process.")
    ...
    try:
        ...
        if request.initial_full_dump:
            ...
            if results and results.get('results'):
                ...
                message = f"Subscription created with ID {subscription_id}. Initial dump of {len(results['results'])} articles scheduled."
            else:

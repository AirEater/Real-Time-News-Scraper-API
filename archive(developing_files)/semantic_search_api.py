...
# --- Configuration ---
EMBEDDING_MODEL = 'BAAI/bge-base-en-v1.5' # Must match the model in the embedding script
...
def get_embedding(text: str, model: SentenceTransformer) -> list[float]:
    ...
    # For BGE models, it is recommended to prepend an instruction to the query.
...
def search_with_tiered_threshold(
    ...
    mongo_collection,
    model: SentenceTransformer, # Pass the loaded model into the function
    query: str, 
    limit: int = -1,
    ...
    # Pre-flight check for relevance
    ...
    if limit == -1:
        # For unlimited requests, search a large, fixed number of candidates
        candidate_limit = 1000
    else:
        # For limited requests, scale the search based on the desired article count
        base_candidate_limit = 150
        multiplier = 20
        candidate_limit = base_candidate_limit + limit * multiplier
        candidate_limit = min(candidate_limit, 2000) # Cap to prevent extreme searches
    ...
    for hit in final_hits:
        # If limit is not -1, stop once we have enough unique articles
...
if __name__ == "__main__":
    # This CLI demo can be extended to test the new company feature.
    pass
